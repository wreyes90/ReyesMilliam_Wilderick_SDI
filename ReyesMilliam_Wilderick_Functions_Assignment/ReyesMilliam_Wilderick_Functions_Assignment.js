...
function lottery(lotteryType, flNumbers, pbNumbers, pbFinal) { // lottery function with the internal variables set as parameters
    var clicks = 1; // internal variable used for the validation if the correct info was typed on the prompt
    ...
    while (lotteryType === "") { // while loop used to validate correct info was typed by the user.
        ...
        clicks ++; // math expression for the clicks variable.

        if (clicks === 4) { // if statement to set up a limit on how many times to loop will run.
    ...
    if (lotteryType === "Power Ball") { // if Statement to run if Power Ball was chosen.
        console.log("The Power Ball winning numbers are:\n" + pbNumbers + " PB " + pbFinal); // Assignment to the console of the message if Power Bal was typed concatenated with the random numbers.
    }
}
